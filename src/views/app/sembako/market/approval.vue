...
export default {
  ...
  methods: {
    ...
    reject(item,index,button,
      ...
      const auth = {'Authorization' : 'Bearer ' + JSON.parse(localStorage.getItem('user')).token}
      let params = item.id
      let promise = axios.patch(apiDev + `/api/v1/acquisition/approval/rejected/${params}`, {}, {
        headers: {'Authorization' : 'Bearer ' + JSON.parse(localStorage.getItem('user')).token}
        }
      )
      return promise
    ...
    dataProvider (ctx) {
      ...
      const auth = {'Authorization' : 'Bearer ' + JSON.parse(localStorage.getItem('user')).token}
      let promise = axios.get('http://devel-apimersubs.vedniretna.com' + `/api/v1/acquisition/dashboard/ownerDetail?pageSize=${size}&page=${current}&keyword=${this.search}&status=${'being proccessed'}`, {headers:auth})
      return promise
