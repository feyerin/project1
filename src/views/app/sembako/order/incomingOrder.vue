...
<div>
  <b-row>
    <b-colxx xxs="12">
        <piaf-breadcrumb :heading="$t('Incoming Order')" />
                ...
                <b-colxx >
                  <div class="d-block d-md-inline-block pt-1">
                        ...
                        <div class="float-right ml-2">
                            <!-- <div xxs="6" class="custom-css">
                                <v-date-picker mode="single" v-model="date" @click="changeDateRange(selectedValueRange)"></v-date-picker>
                            </div> -->
                          </div>
                    </div>
                </b-colxx>
...
<script>
import { urlProd, urlDev, oapiDev, apiUrl, apiDev } from '../../../../constants/config'
...
export default {
  ...
  data () {
    return {
      ...
      bootstrapTable: {
        ...
        selectMode: 'multi',
        fields: [
          { key: 'id', label: 'ID', sortable: false, sortDirection: 'desc', tdClass: 'list-item-heading' },
          { key: 'merchant_name', label: 'Pasar', sortable: false, sortDirection: 'desc', tdClass: 'list-item-heading' },
          { key: 'orders_count', label: 'Jumlah', sortable: false },
  ...
  methods: {
    ...
    dataProvider (ctx) {
      ...
      let promise = axios.get(apiDev + `/api/bank-dki/v1/orders/incoming?page=${current}&limit=${size}&search=${this.search}`, {headers:auth})
      return promise
