...
<div>
  <b-row>
    <b-colxx xxs="12">
        <piaf-breadcrumb :heading="$t('Merchant non-PKP')" />
...
export default {
    ...
    return {
      ...
      bootstrapTable: {
        ...
        fields: [
          ...
          { key: 'owner_address', label: 'Address', sortable: false, size:'20px'},
          { key: 'owner_email_address', label: 'Email', sortable: false },
          { key: 'owner_phone_number', label: 'company phone', sortable: false },
  ...
  methods: {
    ...
    dataProvider (ctx) {
      ...
      let promise = axios.get(urlProd + `/api/v1/acquisition/dashboard/typeMerchant/0?pageNumber=${current}&pageSize=${size}&search=${this.search}`, {headers:auth})
      return promise
